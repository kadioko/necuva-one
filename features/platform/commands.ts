...
export async function setTenantContext(_previousState: PlatformActionState, formData: FormData): Promise<PlatformActionState> { void _previousState; const parsed = tenantContextSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return { status: "error", message: "Review the tenant context." }; const supabase = await createClient(); const { error } = await supabase.rpc("set_tenant_context", { input: parsed.data }); return error ? { status: "error", message: "Tenant context could not be saved." } : { status: "success", message: "Tenant context saved." }; }
