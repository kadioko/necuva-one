...
Organisation owners also manage support access, custom roles, Phase 2 localisation configuration, business-party master data, the item catalogue, and payment references. Tenant localisation separates draft creation (`organisation.localisation.manage`) from approval (`organisation.localisation.approve`); both are initially granted to organisation owners. Party categories, parties, contacts, and addresses require `organisation.parties.manage`; catalogue categories, units, conversions, items, and barcodes require `organisation.catalog.manage`; company payment methods and settlement accounts require the scope-aware `organisation.payment_references.manage`. Platform owners maintain the shared currency reference data through `platform.localisation.manage`. Custom roles are tenant-owned and limited to currently supported organisation permissions; custom roles cannot receive platform privileges.
