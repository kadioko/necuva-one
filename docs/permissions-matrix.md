...
Organisation owners also manage support access, custom roles, and Phase 2 localisation configuration. Tenant localisation separates draft creation (`organisation.localisation.manage`) from approval (`organisation.localisation.approve`); both are initially granted to organisation owners. Platform owners maintain the shared currency reference data through `platform.localisation.manage`. Custom roles are tenant-owned and limited to currently supported organisation permissions; custom roles cannot receive platform privileges.
