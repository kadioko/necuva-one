...
Current automated coverage includes schema validation for provisioning, scoped memberships, invitations, lifecycle updates, plan inputs, currencies, exchange rates, tax configurations, business parties, the item catalogue, and payment references. pgTAP verifies localisation, business-party, catalogue, and company-scoped payment-reference permission boundaries, including audit masking of settlement identifiers. Database tests remain required for every new security-definer RPC; local execution is blocked until Docker Desktop's Linux daemon is available.
