...
Current automated coverage includes schema validation for provisioning, scoped memberships, invitations, lifecycle updates, plan inputs, currencies, exchange rates, tax configurations, business parties, the item catalogue, payment references, and strict CSV parsing. pgTAP verifies localisation, business-party, catalogue, company-scoped payment-reference, and controlled-import permission boundaries. Import coverage includes create/update previews, invalid-row blocking, duplicate detection, atomic confirmation, and batch audit events. Database tests remain required for every new security-definer RPC; local execution is blocked until Docker Desktop's Linux daemon is available.
